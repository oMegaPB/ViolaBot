        ...
        if str(dis) == str(ctx.author):
            ...
            for i in all:
                ...
                try:
                    ...
                    with open(os.path.dirname(os.path.realpath(__file__)).replace('\\Cogs', '') + '\\Config\\assets\\linked.txt', 'a') as file:
                ...
                if i['ctxid'] == ctxid:
                    ...
                    return
            await ctx.send('`Successfully linked your account.`')
            with open(r'C:\Users\Maksim\Desktop\Python\Projects\Viola\Config\assets\linked.txt', 'a') as file:
                file.write(str({"ctxid": ctxid, "uuid": uuid}) + "\n")
        else:
            if dis:
    ...
    async def unlink_account(self, ctx: commands.Context):
        ...
        for i in all:
            ...
            if str(i['ctxid']) == str(ctx.author.id):
                ...
                break
            count += 1
        with open(os.path.dirname(os.path.realpath(__file__)).replace('\\Cogs', '') + '\\Config\\assets\\linked.txt', 'w') as file:
            for j in all:
                file.write(j)
