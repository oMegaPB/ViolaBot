...
class Hypixel(commands.Cog, description='Hypixel related commands.'):
    ...
    async def guild_top(self, ctx, *info):
        if not info:
            embed = discord.Embed(title='Топ гильдии по опыту.', description="Пример: `s!gtop ML`, OR `s!gtop 3 DjagaArmy`", color=discord.Color.green())
        ...
        if int(day) > 7 or int(day) < 1:
            embed = discord.Embed(title='Ошибка', description="Укажите цифры от 1 до 7", color=discord.Color.green())
            await ctx.send(embed=embed)
            return
        embed = discord.Embed(title='Подождите...', description="Собираю информацию о гильдии...", color=discord.Color.green())
        ...
        try:
            ...
            if gexp != None:
                if gexp != 'None':
                    ...
                    embed = discord.Embed(title=f'{name}', description="Мёртвая гильдия.👍", color=discord.Color.green())
            else:
                embed = discord.Embed(title='Ошибка', description="Гильдия не найдена.", color=discord.Color.green())
    ...
    def getDiscord(self, username: str):
        try:
            try:
    ...
    async def link_account(self, ctx: commands.Context, name: str):
        ...
        if str(dis) == str(ctx.author):
            ...
            if dis:
                embed = discord.Embed(title = 'Ошибка', description = f"`Ваш дискорд не совпадет с дискордом игрока {name}`.", color = discord.Color.green())
                await ctx.send(embed = embed)
            else:
                embed = discord.Embed(title = 'Ошибка', description = f"`У {name} не указан дискорд.`", color = discord.Color.green())
                await ctx.send(embed = embed)

    @commands.command(aliases = ['unlink', ])
    async def unlink_account(self, ctx: commands.Context):
        res = await self.bot.bd.remove({"ctxid": ctx.author.id}, category='linked')
        if res.value > 0:
            await ctx.channel.send('`Ваш аккаунт успешно отвязан.`')
        else:
            await ctx.channel.send('`Аккаунт привязанный к этому дискорду не найден в базе.`')
        return
    ...
    async def stats(self, ctx: commands.Context, *username): # [rank, level_full, achi, karma, quests, friends, guild_all, tss, self.kd, names, links, self.uuid]
        ...
        if not username:
            try:
                res = await self.bot.bd.fetch({"ctxid": ctx.author.id}, category='linked')
                if res.status:
                    username = requests.get(f"https://api.mojang.com/user/profiles/{res.value['uuid']}/names").json()[-1]['name']
                else:
                    embed = discord.Embed(title = 'Ошибка', description = 'Укажите никнейм или привяжите свой аккаунт: s!link <username>', color = discord.Color.green())
                    await ctx.send(embed = embed)
                    return
            except (KeyError, discord.errors.Forbidden) as e:
                print(e, 'stats error')
                return
        try:
            ...
            embed = discord.Embed(title = 'Ошибка', description = 'Такого никнейма нет.', color = discord.Color.green())
            await ctx.send(embed=embed)
    # --------------------------------------------------------------------------------------------------
    class Stats:
